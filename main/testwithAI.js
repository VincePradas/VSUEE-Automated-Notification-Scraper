async function works(event, api) {
	...
	async function scrapeNotifications() {
		...
		const parsedNotifications = notifications.map((notification) => {
			...
			const course = $("td:contains('Course:')").text().replace("Course:", "").trim(); // Selector for course info
		...
		notifications.forEach((notification) => {
			...
			if (delay > 0) {
				setTimeout(() => {
                    ...
                    api.sendMessage("Scraped Notifications:", event.threadID, event.messageID);
					api.sendMessage(notification.message, event.threadID, event.messageID);
				}, delay);
			} else {
